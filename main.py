...
if platform == 'android':
    try:
        ...
        # Android 12+ માટે "Allow" વાળું પોપઅપ લાવવા માટેનો કોડ
        try:
    ...
    def toggle_bluetooth(self):
        ...
        try:
            ...
            if not paired_devices:
                self.messagebox_kivy("No Paired Devices", "કોઈ પેર કરેલું બ્લૂટૂથ ડિવાઇસ મળ્યું નથી. કૃપા કરીને સેટિંગ્સમાં જઈને પેર કરો.")
                return

            # નવું ફિચર: સ્કેન ફિલ્ટર કાઢી નાખ્યું અને ડિવાઇસ પસંદ કરવા પોપઅપ ઉમેર્યું
            ...
            for d in paired_devices:
                ...
                btn.bind(on_release=lambda instance, selected_device=d: self.connect_to_selected_device(selected_device))
                grid.add_widget(btn)
            ...
            self.messagebox_kivy("Error", str(e))

    def connect_to_selected_device(self, device):
        if self.device_popup:
            self.device_popup.dismiss()

        try:
            adapter = BluetoothAdapter.getDefaultAdapter()
            # 🚀 FIX: કનેક્ટ કરતા પહેલા Discovery બંધ કરવી ફરજિયાત છે, નહીંતર JVM એરર આવે છે.
            ...
            try:
                # પહેલા સ્ટાન્ડર્ડ કનેક્શન ટ્રાય કરશે
                self.bt_socket = device.createRfcommSocketToServiceRecord(s_uuid)
                self.bt_socket.connect()
            except Exception as e1:
                print(f"Standard connection failed: {e1}. Trying insecure connection...")
                # 🚀 FIX: જો ફોટામાં આપેલી "read failed" એરર આવે તો આ બેકઅપ કનેક્શન ચાલશે.
            ...
            self.disconnect_bluetooth()
            # જો એરર આવે તો હવે તે તમને એક્ઝેટ મેસેજ પોપઅપમાં બતાવશે.
